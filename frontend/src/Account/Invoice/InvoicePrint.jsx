...
const InvoicePrint = ({ invoiceData }) => {
  ...
  const subtotal = items.reduce(
    (prev, curr) => prev + (Number(curr.price) || 0) * (curr.qty || 0),
    0
  );
    ...
    <div className="p-2 bg-white rounded shadow-md">
      ...
      <div className="grid grid-cols-12 border border-gray-300">
        ...
        <div className="col-span-7 p-2 space-y-3 border-r border-gray-200">
          ...
          <p className="text-sm">
            CIN:{" "}
            <span className="font-semibold">{companyInfo.cin || "N/A"}</span>
          </p>
          <p className="text-sm">
            E-Mail:{" "}
            <span className="font-semibold">{companyInfo.email || "N/A"}</span>
          </p>
          <div className="pt-2 border-t border-gray-300">
            ...
            <p className="text-sm">
              Name: {buyerDetails.customerName || "N/A"}
            </p>
      ...
      <div className="pb-2">
        <table className="w-full border border-collapse table-auto border-slate-500">
          <thead>
            <tr className="text-sm ">
              ...
              <th className="p-2 text-center border border-slate-600">
                HSN/SAC
              </th>
              <th className="p-2 text-center border border-slate-600">
                Quantity
          ...
          <tbody>
            {items.map((item, index) => {
              const itemTaxPrice =
                (totalTaxRate * Number(item.price || 0)) / 100;
        ...
        <div className="py-2 border border-gray-300">
          <div className="flex justify-between">
            ...
            <span className="mr-4">
              ₹{((cgst * subtotal) / 100).toFixed(2)}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="ml-4">SGST:</span>
            <span className="mr-4">
              ₹{((sgst * subtotal) / 100).toFixed(2)}
            </span>
          </div>
          <div className="flex justify-between">
        ...
        <div className="flex flex-wrap border border-gray-300">
          <div className="flex-1 border-r border-gray-300">
            ...
            {items &&
              items.map((item, index) => (
                <span key={index} className="block ml-2 border-b">
                  {index + 1}. {item.hsn}
                </span>
              ))}
          </div>
          <div className="flex-1 border-r border-gray-300">
            <p className="text-center border-b border-gray-300">
              Taxable Value
            </p>
        ...
        <div className="border-gray-300 border-x">
          ...
          <div className="flex flex-auto">
            <div className="border border-gray-300 ">
              ...
              <p className="mx-2 text-wrap">
                We declare that this invoice the actual price of the goods
                described and that all particulars are true and correct.
              </p>
            </div>
            <div className="border-b border-gray-300">
              <p className="mt-2 ml-2 text-sm font-semibold">
                For {companyInfo.companyName}
              </p>
              <p className="pb-2 mt-4 mr-2 text-sm font-semibold text-end">
