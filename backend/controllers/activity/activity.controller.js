...
const createActivity = async (req, res) => {
  try {
    ...
    formData.image = req.file ? `${baseUrl}${req.file.filename}` : formData.image;

    // Create a new activity with the processed formData
    const newActivity = new Activity(formData);

    await newActivity.save();
    res.status(201).json({ success: true, message: 'Activity created successfully', activity: newActivity });
  } catch (error) {
...
const updateActivity = async (req, res) => {
    try {
      ...
      const updatedActivity = await Activity.findByIdAndUpdate(id, formData, { new: true });
      
      if (!updatedActivity) {
        return res.status(404).json({ success: false, message: 'Activity not found' });
      }
  
      res.status(200).json({ success: true, message: 'Activity updated successfully', activity: updatedActivity });
    } catch (error) {
