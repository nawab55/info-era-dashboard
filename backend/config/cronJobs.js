...
const autoCheckout = async () => {
    try {
      ...
      console.log(`Found ${pendingCheckouts.length} pending checkouts. Performing automatic checkout.`);
      ...
      for (const attendance of pendingCheckouts) {
        ...
        const checkInDateTimeStr = `${attendance.date} ${format(attendance.checkInTime, "hh:mm a")}`;
        const checkInDateTime = parse(checkInDateTimeStr, "dd/MM/yyyy hh:mm a", new Date());
        ...
        console.log(`Automatically checked out user ${attendance.userId} on ${todayDate}.`);
      }
    } catch (error) {
